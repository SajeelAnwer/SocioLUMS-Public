...
export default function SocietyLogin({navigation, route}) {
  ...
  const handleLogin = () => {
    if(email == "" || password == ""){
      ...
      setloading(true)
      fetch('https://sociolums-backend.up.railway.app/loginsociety', {
        method: 'post',
        headers: {
      ...
      .then(
        async data => {
            if(data.error === "Invalid Email Address"){
                setloading(false)
                alert('Please enter a valid LUMS Society Email: xxxxx@lums.edu.pk')
            }
                ...
                setloading(false)
                await AsyncStorage.setItem('society', JSON.stringify(data)) //saves msg token user: id,email,acctype in Async storage
                navigation.navigate('SocietyMain', {data})
            }
            else{
                //console.log("here2")
                setloading(false)
  ...
  const navigateToFP_Email =  () => { navigation.navigate('Society_FP_email') }
