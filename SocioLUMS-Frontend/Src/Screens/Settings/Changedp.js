...
export default function Changedp({navigation}) {
    
    const [loading, setloading] = useState(false);
    const [student_dp, setstudent_dp] = useState('');
    ...
    const handlechangedp = () => {
        AsyncStorage.getItem('student').then(data => {
            ...
            .then(url => {
                if(url){
                    fetch('https://sociolums-backend.up.railway.app/changedp', {
                        method: 'post',
                        headers: {
