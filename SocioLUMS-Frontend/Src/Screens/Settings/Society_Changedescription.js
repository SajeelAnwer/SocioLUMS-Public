...
export default function Society_Changedescription({navigation}) {
    
    const [loading, setloading] = useState(false);

    const [Society_description, setdesc] = useState('')
    
    
    const handlechangedesc = () => {
        if(!Society_description){
            alert('Please add all the fields')
        }
        else{
            setloading(true)
            AsyncStorage.getItem('society').then(data => {
                fetch('https://sociolums-backend.up.railway.app/societychangedescription', {
                    method: 'post',
                    headers: {
                ...
                .then(data => {
                    if(data.message === "Description updated successfully"){
                        alert('Society Description has been updated successfully')
