...
export default function Society_Changedp({navigation}) {
    
    const [loading, setloading] = useState(false);
    const [Society_dp, setsociety_dp] = useState('');
    ...
    const handlechangedp = () => {
        AsyncStorage.getItem('society').then(data => {
            ...
            .then(url => {
                if(url){
                    fetch('https://sociolums-backend.up.railway.app/societychangedp', {
                        method: 'post',
                        headers: {
