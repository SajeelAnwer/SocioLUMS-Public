...
export default function Sponsor_Changecover({navigation}) {
    
    const [loading, setloading] = useState(false);
    const [Sponsor_cover, setsponsor_cover] = useState('');
    ...
    const handlechangecover = () => {
        AsyncStorage.getItem('sponsor').then(data => {
            ...
            .then(url => {
                if(url){
                    fetch('https://sociolums-backend.up.railway.app/sponsorchangecover', {
                        method: 'post',
                        headers: {
                    ...
                    .then(data => {
                        if(data.message == "Image uploaded successfully"){
                            alert('Your Cover Photo has been changed successfully')
      ...
      <View style={styles.container}>
        <ImageBackground source={require('../../../assets/splash.png')} resizeMode="cover" style={styles.background}>
          ...
          
            {
              ...
              :
              <TouchableOpacity style={styles.button} onPress={()=>handlechangecover()}>
