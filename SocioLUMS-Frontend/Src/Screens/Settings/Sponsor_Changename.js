...
export default function Sponsor_Changename({navigation}) {
    
    const [loading, setloading] = useState(false);

    const [Sponsor_name, setname] = useState('')
    ...
    const handlechangename = () => {
        if(!Sponsor_name || !password){
            ...
            AsyncStorage.getItem('sponsor').then(data => {
                fetch('https://sociolums-backend.up.railway.app/sponsorchangename', {
                    method: 'post',
                    headers: {
                ...
                .then(data => {
                    if(data.error === "Wrong password"){
                        ...
                        alert('Your Sponsor name has been changed successfully')
